...
patience = 200
ITER = 10  # 训练-测试 轮数
img_channels = data_IN.shape[-1]  # PCA降维后的维度数        可尝试论文的15
ValMode = "none"  # 验证集模式
...
# 将图像拉成向量进行标准化后再还原进行PCA降维（gabor的matlab操作已完成PCA和归一化过程）
data = data_IN
...
for index_iter in np.arange(ITER):
    ...
    history_res4_SS_BN = model_res4_SS_BN.fit(
        ...
        nb_epoch=nb_epoch, shuffle=True, callbacks=[earlyStopping6, saveBestModel6, callbacks_list, history])
    toc6 = time.clock()
    ...
    # sio.savemat('./loss_' + str(DataSetName) + '_' + str((index_iter + 1)) + '.mat', {'losses':history.losses})
...
modelStatsRecord.outputStats(KAPPA_RES_SS4, OA_RES_SS4, AA_RES_SS4, ELEMENT_ACC_RES_SS4,
                             ...
                             'records/' + str(save_name) + '_pf_cnn.txt',
